#!/usr/bin/env python3
# -*- coding: utf-8 -*-

import argparse
import json
import os
import numpy as np
import matplotlib.pyplot as plt
from matplotlib.ticker import AutoMinorLocator


def set_paper_style():
    plt.rcParams.update({
        "figure.dpi": 120,
        "savefig.dpi": 300,
        "font.size": 11,
        "axes.titlesize": 12,
        "axes.labelsize": 11,
        "legend.fontsize": 10,
        "xtick.labelsize": 10,
        "ytick.labelsize": 10,
        "axes.grid": True,
        "grid.alpha": 0.25,
        "grid.linestyle": "--",
        "axes.spines.top": False,
        "axes.spines.right": False,
    })


def mean_std(x):
    a = np.array(x, dtype=np.float64)
    return float(a.mean()), float(a.std(ddof=1) if a.size > 1 else 0.0)


def plot_bar(methods, rmses, out_path):
    means, stds = [], []
    for m in methods:
        mu, sd = mean_std(rmses[m])
        means.append(mu); stds.append(sd)

    x = np.arange(len(methods))
    fig = plt.figure(figsize=(8.6, 3.6))
    ax = fig.add_subplot(111)
    ax.bar(x, means, yerr=stds, capsize=4)
    ax.set_xticks(x)
    ax.set_xticklabels(methods, rotation=20, ha="right")
    ax.set_ylabel("Test RMSE (mean ± std, 5 folds)")
    ax.set_title("Matrix Factorization Methods: 5-fold RMSE")
    ax.yaxis.set_minor_locator(AutoMinorLocator())
    ax.grid(True, axis="y", which="both", linestyle="--", alpha=0.25)
    fig.tight_layout()
    fig.savefig(out_path, bbox_inches="tight")
    plt.close(fig)


def plot_box(methods, rmses, out_path):
    data = [rmses[m] for m in methods]
    fig = plt.figure(figsize=(8.6, 3.6))
    ax = fig.add_subplot(111)
    ax.boxplot(data, labels=methods, showmeans=True)
    ax.set_ylabel("Test RMSE")
    ax.set_title("RMSE distribution across folds")
    plt.setp(ax.get_xticklabels(), rotation=20, ha="right")
    ax.yaxis.set_minor_locator(AutoMinorLocator())
    ax.grid(True, axis="y", which="both", linestyle="--", alpha=0.25)
    fig.tight_layout()
    fig.savefig(out_path, bbox_inches="tight")
    plt.close(fig)


def plot_heatmap(methods, rmses, out_path):
    k = len(next(iter(rmses.values())))
    mat = np.zeros((k, len(methods)), dtype=np.float64)
    for j, m in enumerate(methods):
        mat[:, j] = np.array(rmses[m], dtype=np.float64)

    fig = plt.figure(figsize=(8.6, 3.8))
    ax = fig.add_subplot(111)
    im = ax.imshow(mat, aspect="auto", interpolation="nearest")
    ax.set_xticks(np.arange(len(methods)))
    ax.set_xticklabels(methods, rotation=20, ha="right")
    ax.set_yticks(np.arange(k))
    ax.set_yticklabels([f"fold{i}" for i in range(k)])
    ax.set_title("Per-fold test RMSE heatmap")
    cbar = fig.colorbar(im, ax=ax, fraction=0.046, pad=0.02)
    cbar.set_label("RMSE")
    fig.tight_layout()
    fig.savefig(out_path, bbox_inches="tight")
    plt.close(fig)


def plot_rankline(methods, rmses, out_path):
    k = len(next(iter(rmses.values())))
    fig = plt.figure(figsize=(8.6, 3.6))
    ax = fig.add_subplot(111)

    for fi in range(k):
        pairs = [(m, rmses[m][fi]) for m in methods]
        pairs.sort(key=lambda x: x[1])
        ys = [p[1] for p in pairs]
        ax.plot(np.arange(len(methods)), ys, marker="o", linewidth=1.5, label=f"fold{fi}")

    ax.set_xticks(np.arange(len(methods)))
    ax.set_xticklabels([str(i + 1) for i in range(len(methods))])
    ax.set_xlabel("Within-fold rank (1=best)")
    ax.set_ylabel("Test RMSE")
    ax.set_title("Stability view: RMSE vs within-fold rank")
    ax.yaxis.set_minor_locator(AutoMinorLocator())
    ax.grid(True, which="both", linestyle="--", alpha=0.25)
    ax.legend(ncol=3, frameon=False)
    fig.tight_layout()
    fig.savefig(out_path, bbox_inches="tight")
    plt.close(fig)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--results_json", type=str, required=True, help="path to folds_hash/results.json")
    ap.add_argument("--out_dir", type=str, default=None)
    ap.add_argument("--method_order", type=str, nargs="*", default=None)
    args = ap.parse_args()

    set_paper_style()

    with open(args.results_json, "r", encoding="utf-8") as f:
        obj = json.load(f)
    rmses = obj["results"]
    methods = args.method_order if args.method_order else list(rmses.keys())

    out_dir = args.out_dir or os.path.dirname(os.path.abspath(args.results_json))
    os.makedirs(out_dir, exist_ok=True)

    plot_bar(methods, rmses, os.path.join(out_dir, "rmse_bar_mean_std.png"))
    plot_box(methods, rmses, os.path.join(out_dir, "rmse_boxplot.png"))
    plot_heatmap(methods, rmses, os.path.join(out_dir, "rmse_heatmap.png"))
    plot_rankline(methods, rmses, os.path.join(out_dir, "rmse_rankline.png"))

    print("Saved figures to:", out_dir)


if __name__ == "__main__":
    main()
